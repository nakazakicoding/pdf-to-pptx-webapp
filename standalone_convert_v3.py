"""
PDF to PowerPoint Converter v42 (subsubOverlay)
- v41ベース
- v42: L2処理でサブサブ要素をオーバーレイに追加。サブ親はスキップ。サブ親の50%以上の巨大サブサブは除外。
"""
import os
import sys
import fitz  # PyMuPDF
...
def load_ocr_corrections_v2():
    ...
    json_path = os.path.join(base_dir, "temp_processing", "129493ae-b15b-4089-9e12-07d76f8f3603", "image_analysis.json")
    ...
    if os.path.exists(json_path):
        ...
        print(f"Warning: all_pages_ocr_v2.json not found at {json_path}")
        return {}
                    ...
                    if not (first_match and last_match):
                        ...
                        if ocr_len > 0 and json_len > ocr_len:
                            ...
                            if first_match and not last_match:
                                ...
                                # ここでは「左はJSON起点」の原則を守りつつ、文字分だけ左にシフト(JSONより左へ)する
                                # ただし、グラフィック消去リスクがあるため慎重に。
...
def add_slide_from_page(prs, page, page_image_path, page_width, page_height):
    ...
    # ★v2新機能: サブ親要素が検出されたページでは元画像バックアップレイヤーを追加
    # ★v3新機能: 親要素（通常グラフィック要素）が一定数以上検出されたページにも追加
    has_sub_parent = len(sub_overlay_elements) > 0
    has_parent_elements = len(regular_elements) >= 3  # v3: 3個以上の通常グラフィックがあれば複雑なデザイン
    should_add_backup = has_sub_parent or has_parent_elements
    
    if should_add_backup:
        reason = "Sub-parent" if has_sub_parent else f"Parent elements ({len(regular_elements)})"
        print(f"  [v3 BACKUP] {reason} detected! Adding original image backup layer...")
        
        # 1. 元画像を一番後ろに追加（スライド全体サイズ）
        try:
            original_pic = slide.shapes.add_picture(
                page_image_path, 
                Pt(0), Pt(0), 
                Pt(page_width), Pt(page_height)
            )
            # 元画像を一番後ろに移動
            # PowerPointでは shapes はインデックス順に描画される（小さいほど後ろ）
            # 新しく追加した shapes を最後に移動するには...実際は追加順なので問題ない
            print(f"  [v3 BACKUP] Added original image at back")
        except Exception as e:
            print(f"  [v3 BACKUP ERROR] Failed to add original image: {e}")
        
        # 2. 背景色の無地四角を追加（元画像の上に重ねる）
        try:
            ...
            print(f"  [v3 BACKUP] Added background overlay with color RGB({bg_color[0]},{bg_color[1]},{bg_color[2]})")
        except Exception as e:
            print(f"  [v3 BACKUP ERROR] Failed to add background overlay: {e}")
...
def main():
    ...
    final_output_path = os.path.join(final_output_dir, "webapp_test_output4.pptx")
...
if __name__ == "__main__":
    # ログをファイルに出力（行バッファリング buffering=1）
    import sys
    log_file = open("debug_log_beans.txt", "w", encoding="utf-8", buffering=1)
    sys.stdout = log_file
    try:
        main()
    finally:
