...
def load_ocr_corrections_v2():
    ...
    if CURRENT_JSON_PATH and os.path.exists(CURRENT_JSON_PATH):
        ...
        json_path = os.path.join(base_dir, "temp_processing", "a9fccef7-648c-4def-b5a0-1d04c5c10eb5", "image_analysis.json")
...
def add_slide_from_page(prs, page, page_image_path, page_width, page_height):
    ...
    # ★v4新機能: 全ページに元画像バックアップレイヤーを追加
    print(f"  [v4 BACKUP] Adding original image backup layer to ALL pages...")
    ...
    try:
        ...
        print(f"  [v4 BACKUP] Added original image at back")
    except Exception as e:
        print(f"  [v4 BACKUP ERROR] Failed to add original image: {e}")
    ...
    try:
        ...
        print(f"  [v4 BACKUP] Added background overlay with color RGB({bg_color[0]},{bg_color[1]},{bg_color[2]})")
    except Exception as e:
        print(f"  [v4 BACKUP ERROR] Failed to add background overlay: {e}")
...
def main(pdf_path=None, output_path=None, json_path=None):
    ...
    # デフォルト値
    if pdf_path is None:
        pdf_path = r"c:\Users\ishik\Documents\Antigravity\PDFパワポ化\webapp\temp_processing\a9fccef7-648c-4def-b5a0-1d04c5c10eb5\input.pdf"
    if output_path is None:
        final_output_dir = r"c:\Users\ishik\Documents\Antigravity\PDFパワポ化\output"
        os.makedirs(final_output_dir, exist_ok=True)
        output_path = os.path.join(final_output_dir, "webapp_test_output_v6.pptx")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="PDF to PPTX Converter - Safeguard Mode (with backup layers)")
