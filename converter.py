...
def load_ocr_corrections_v2():
    ...
    json_path = os.path.join(base_dir, "test", "image_analysis.json")
    ...
    for i, block in enumerate(text_blocks):
        ...
        if "word_details" in block and block["word_details"] and cv_img is not None:
            ...
            if cv_img is not None:
                ...
                elif use_width_based_size:
                    box_width_pt = bbox[2] - bbox[0]
                    text_content = block["text"].replace(" ", "").replace("\n", "")
                    char_count = len(text_content)
                    
                    if char_count > 0 and box_width_pt > 0:
                        width_based_size = (box_width_pt / char_count) * 0.95
                        ssim_size = font_props["font_size"]
                        width_based_size = min(width_based_size, 100.0)
                        
                        if ssim_size < width_based_size * 0.5 or ssim_size > width_based_size * 2.0:
                            final_font_size_pt = width_based_size
                            print(f"    [WIDTH] SSIM discarded ({ssim_size:.1f}pt), using width-based: {final_font_size_pt:.1f}pt")
                        else:
                            final_font_size_pt = width_based_size * 0.7 + ssim_size * 0.3
                            print(f"    [WIDTH] Blended: width={width_based_size:.1f}pt, SSIM={ssim_size:.1f}pt -> {final_font_size_pt:.1f}pt")
                    else:
...
def main():
    """メイン処理 - 全ページ変換"""
    pdf_path = r"c:\Users\ishik\Documents\Antigravity\PDFパワポ化\Source\The_Longevity_Bean.pdf"
    
    final_output_dir = r"c:\Users\ishik\Documents\Antigravity\PDFパワポ化\output"
    os.makedirs(final_output_dir, exist_ok=True)
    final_output_path = os.path.join(final_output_dir, "The_Longevity_Bean.pptx")
